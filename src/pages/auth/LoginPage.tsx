...
const schema = z.object({
  ...
  password: z.string().min(1, 'login.password_required'),
})

type FormData = z.infer<typeof schema>

const FEATURES = [
  { icon: BarChart3, key: 'login.feature_pipeline' },
  { icon: Globe, key: 'login.feature_landing' },
  { icon: Zap, key: 'login.feature_ai' },
  { icon: Shield, key: 'login.feature_tasks' },
]
...
export function LoginPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    catch (err) { setError(err instanceof Error ? err.message : t('login.connection_error')) }
    ...
    <div className="flex min-h-screen" style={{fontFamily:"'Inter',-apple-system,sans-serif"}}>
      ...
      <div className="hidden w-[480px] shrink-0 flex-col justify-between overflow-hidden p-10 text-white lg:flex" style={{background:'linear-gradient(165deg,#050D1A 0%,#0A2540 35%,#0B3D6F 65%,#0579DA 100%)',position:'relative'}}>
        ...
        <div style={{position:'relative',zIndex:1}}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/40">{t('login.tagline')}</p>
            </div>
          </div>
          <h2 className="mt-8 text-[28px] leading-tight" style={{fontWeight:900}}>
            {t('login.hero_title_1')}<br/>
            <span style={{background:'linear-gradient(135deg,#3BA3FF,#06B6D4)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>{t('login.hero_title_2')}</span>
          </h2>
        ...
        <div style={{position:'relative',zIndex:1}} className="space-y-3">
          {FEATURES.map(f => (
            <div key={f.key} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.04] px-4 py-3 backdrop-blur-sm">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10">
                <f.icon className="h-4 w-4 text-white/80" />
              </div>
              <span className="text-[13px]" style={{fontWeight:500,color:'rgba(255,255,255,.8)'}}>{t(f.key)}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{position:'relative',zIndex:1}} className="space-y-4">
          ...
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-4 backdrop-blur-sm">
            ...
            <p className="text-[12px] leading-relaxed text-white/55 italic">{t('login.testimonial')}</p>
            <div className="mt-3 flex items-center gap-2">
              ...
              <div><div className="text-[11px]" style={{fontWeight:600}}>{t('login.testimonial_author')}</div><div className="text-[9px] text-white/30">{t('login.testimonial_role')}</div></div>
            </div>
          </div>
          {/* Stats */}
          <div className="flex gap-6">
            <div><div className="text-lg" style={{fontWeight:900}}>+107</div><div className="text-[9px] text-white/30">{t('login.stat_sales')}</div></div>
            <div><div className="text-lg" style={{fontWeight:900}}>+15K</div><div className="text-[9px] text-white/30">{t('login.stat_leads')}</div></div>
            <div><div className="text-lg" style={{fontWeight:900}}>87%</div><div className="text-[9px] text-white/30">{t('login.stat_satisfaction')}</div></div>
          </div>
          <p className="text-[9px] text-white/15">{t('login.copyright')}</p>
        </div>
      </div>
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="rounded-2xl border border-[#E3E8EF] bg-white p-8 shadow-xl shadow-black/[0.03] sm:p-10">
            ...
            <div className="mb-6 hidden lg:block">
              ...
              <p className="mt-1 text-[14px] text-[#8898AA]">{t('login.subtitle')}</p>
            </div>
