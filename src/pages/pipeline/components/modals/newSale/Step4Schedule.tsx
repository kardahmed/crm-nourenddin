    ...
    <div className="space-y-5">
      ...
      {!formData.installments ? (
        ...
        <div className="space-y-4">
          {/* Frequency */}
          <div>
            <Label className={labelClass}>{t('sale_modal.frequency')}</Label>
            <div className="mt-1 flex gap-2">
              {([
                { value: 'monthly' as const, label: t('sale_modal.freq_monthly') },
                { value: 'quarterly' as const, label: t('sale_modal.freq_quarterly') },
                { value: 'semiannual' as const, label: t('sale_modal.freq_semiannual') },
              ]).map((f) => (
                <button
                  key={f.value}
                  type="button"
            ...
            {schedule.length > 0 && (
              <p className="mt-1 text-[11px] text-immo-accent-green">{t('sale_modal.installments_planned', { count: schedule.length })}</p>
            )}
          </div>

          {/* Down payment slider */}
          <div>
            <Label className={labelClass}>{t('sale_modal.down_payment')}</Label>
          ...
          {/* First payment date */}
          <div>
            <Label className={labelClass}>{t('sale_modal.first_payment_date')}</Label>
            <Input
              type="date"
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-immo-bg-card-hover">
                    {[t('sale_modal.th_num'), t('sale_modal.th_date'), t('sale_modal.th_amount'), t('sale_modal.th_description')].map((h) => (
          ...
          {formData.deliveryDate && (
            <p className="text-[11px] text-immo-text-muted">
              {t('sale_modal.last_payment_at', { date: format(new Date(formData.deliveryDate), 'dd/MM/yyyy') })}
            </p>
          )}
        </div>
      )}
